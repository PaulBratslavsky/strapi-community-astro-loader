...
describe("astroLoader", () => {
  describe("#load", () => {
    ...
    describe("with stale sync", async () => {
      ...
      it("should fetch content types", async () => {
        ...
        expect(context.store.set).toHaveBeenNthCalledWith(
          1,
          expectedDataStoreEntries[0],
        );
        expect(context.store.set).toHaveBeenNthCalledWith(
          2,
          expectedDataStoreEntries[1],
        );
        expect(context.meta.set).toHaveBeenCalledExactlyOnceWith(
          "lastSynced",
          expect.any(String),
        );
      });
  ...
  describe("#schema", () => {
    it("should throw Error if default schema is used", () => {
      const { schema } = strapiLoader({ contentType: TYPE });
